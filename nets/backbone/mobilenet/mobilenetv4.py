...
import math
from nets.backbone.mobilenet.model_config import MODEL_SPECS
...
class MobileNetV4(nn.Module):
    def __init__(self, model):
        super().__init__()
        assert model in MODEL_SPECS.keys(), "Invalid model type"
        self.model = model
...
def mobilenet_v4( model='MobileNetV4ConvSmall',pretrained=False,):
    # Initialize the MobileNetV4 model
    model = MobileNetV4(model=model)
    if pretrained:
